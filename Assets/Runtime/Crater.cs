...
public class Crater
{
  ...
  public Crater(string name, float diameter, float longitude, float latitude, float sphereRadius, PlaneOrientation orientation)
  {
    ...
    Longitude = longitude;
    Latitude = latitude;
    Coordinate = new GeoCoord(new Vector2(longitude, latitude), orientation, sphereRadius);
  }
  ...
  {
    Position = Vector3.Lerp(Coordinate.worldPosition, Coordinate.spherePosition, lerp);
  }
  ...
  public void UpdateCrater(PlaneOrientation orientation, float radius)
  {
    if (Coordinate == null)
    {
      Coordinate = new GeoCoord(new Vector2(Longitude, Latitude), orientation, radius);
    }
    Coordinate.UpdateCoordinate(orientation, radius);
  }
}